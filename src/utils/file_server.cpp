...
void serve_file(Response& res, fs::path path) {
    ...
    if (!file.is_open() || !file.good()) {
        ...
        res.set_body("Failed to open file");
...
void FileServer::put(const Request& req, Response& res, fs::path path) {
    ...
    if (dir && fs::exists(path)) {
        ...
        res.set_body("Directory already exists");
        res.headers.set("Content-Type", "text/plain");
        return;
    }
    ...
    if (fs::exists(path)) {
        ...
        if (!file.is_open() || !file.good()) {
            res.status = StatusCode::INTERNAL_SERVER_ERROR;
            res.set_body("Failed to open file for writing");
            res.headers.set("Content-Type", "text/plain");
            return;
        }
        file << req.body;
        ...
        if (!file.is_open() || !file.good()) {
            res.status = StatusCode::INTERNAL_SERVER_ERROR;
            res.set_body("Failed to create file");
            res.headers.set("Content-Type", "text/plain");
            return;
        }
        file << req.body;
...
void FileServer::del(const Request& req, Response& res, fs::path path) {
    ...
    if (ec) {
        res.status = StatusCode::INTERNAL_SERVER_ERROR;
        res.set_body("Failed to delete file or directory");
        res.headers.set("Content-Type", "text/plain");
        return;
    }
